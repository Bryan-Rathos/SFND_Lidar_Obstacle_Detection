...
void Proximity(uint index, const std::vector<std::vector<float>> &points, std::vector<int> &newCluster, std::vector<bool> &processed, KdTree* tree, const float &distanceTol)
{
	processed[index] = true;
	newCluster.push_back(index);
	std::vector<int> nearbyID = tree->search(points[index], distanceTol);

	// std::cout << "Point: "  << points[index][0] << ", " << points[index][1] << std::endl;
...
void insertPointsKDTree(std::vector<std::vector<float>> &points, KdTree* tree)
{
    ...
    for (int dim=0; dim<cloud_size; dim++)
	{
		...
		if(dim%2 == 0)
		{
			...
			if(size%2 == 0)
			{
				...
				std::cout << dim << ": "<<medianPt[0] << ", " << medianPt[1] << std::endl;
...
int main ()
{
	...
	// auto endTime = std::chrono::steady_clock::now();
  	...
  	// Render clusters
  	int clusterId = 0;
	std::vector<Color> colors = {Color(1,1,0), Color(0,1,0), Color(1,0,1)};
